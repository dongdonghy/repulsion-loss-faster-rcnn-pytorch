...
class Bottleneck_dilation(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None):
    ...
    self.bn1 = nn.BatchNorm2d(planes)
    self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, dilation=2, # change
...
class ResNet(nn.Module):
  ...
  def _make_layer(self, i, block, planes, blocks, stride=1):
    downsample = None
    ...
    layers = []
    if i!=3:
        layers.append(block(self.inplanes, planes, stride, downsample1))
    else:
        layers.append(Bottleneck_dilation(self.inplanes, planes, 1, downsample2))
...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False):
    self.model_path = '/data-sdb/data/hongyi.dong/ped_detection/pretrained_model/resnet50-caffe.pth'
