...
import math
from repulsion_loss import repulsion
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        if self.training:
            ...
            for i in range(rois_label.shape[0]):
                loss_cls[i]=F.cross_entropy(cls_score[i].view(1,2), rois_label[i].view(1))


            RCNN_loss_cls = torch.mean(torch.cat((loss_cls[rois_label==0].sort(0,True)[0][:int(loss_cls[rois_label==0].size(0)*3/10)], loss_cls[rois_label==1]), 0))            
            ...
            RCNN_loss_repgt, RCNN_loss_repbox = repulsion(rois, bbox_pred, gt_rois, rois_inside_ws, rois_outside_ws)
