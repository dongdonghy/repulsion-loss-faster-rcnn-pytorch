...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    return im
'''
def vis_detections(im, class_ids, image_ids, dets, thresh=0.8):
    """Visual debugging of detections."""
    res=[]
    #import pdb
    #pdb.set_trace()
    for i in range(dets.shape[0]):
        ...
        dict_single={'image_id':image_ids,'category_id':class_ids,'bbox':bbox_wh,'score':float(score)}
        res.append(dict_single)
    print len(res)
    for i in range(dets.shape[0]):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
        if score > thresh:
            if class_ids==1:
                cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 0, 255), 2)
                cv2.putText(im, '%s: %.3f' % (str(class_ids), score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN,
